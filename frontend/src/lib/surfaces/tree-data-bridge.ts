...
import type { TreeDataWindowCommand } from '../components/tree-data/tree-data-controller';
import type { TreeDataWindowSnapshot } from '../components/tree-data/tree-data-transport';
...
export interface SurfaceTransportBridgeCommandEnvelope<CommandPayload> extends SurfaceEnvelopeBase<'surface.command', CommandPayload> {
  expectedRevision?: number;
}

export interface SurfaceTransportBridgeLifecycleEnvelope extends SurfaceEnvelopeBase<'surface.lifecycle', SurfaceLifecycleEvent> {
  expectedRevision?: number;
}

export type TreeDataBridgeSession = SurfaceTransportSession<TreeDataWindowCommand, TreeDataWindowSnapshot>;

export function getTreeDataBridgeSnapshotEventName(instanceId: SurfaceInstanceId): string {
  return `${BRIDGE_SNAPSHOT_EVENT_PREFIX}${instanceId}`;
}

export function isTreeDataBridgeCommandEnvelope(
  envelope: SurfaceEnvelopeBase<'surface.command', TreeDataWindowCommand> | SurfaceTransportBridgeLifecycleEnvelope,
): envelope is SurfaceTransportBridgeCommandEnvelope<TreeDataWindowCommand> {
  return envelope.kind === 'surface.command';
}

export function isTreeDataBridgeLifecycleEnvelope(
  envelope: SurfaceEnvelopeBase<'surface.command', TreeDataWindowCommand> | SurfaceTransportBridgeLifecycleEnvelope,
): envelope is SurfaceTransportBridgeLifecycleEnvelope {
  return envelope.kind === 'surface.lifecycle';
}

export function createTreeDataBridgeSession(
  surfaceId: SurfaceId,
  instanceId: SurfaceInstanceId,
): TreeDataBridgeSession {
  const snapshotListeners = new Set<SurfaceTransportListener<SurfaceEnvelopeBase<'surface.snapshot', TreeDataWindowSnapshot>>>();
  const lifecycleListeners = new Set<SurfaceTransportListener<SurfaceEnvelopeBase<'surface.lifecycle', SurfaceLifecycleEvent>>>();
  const commandListeners = new Set<SurfaceTransportListener<SurfaceEnvelopeBase<'surface.command', TreeDataWindowCommand>>>();
  const errorListeners = new Set<SurfaceTransportListener<SurfaceEnvelopeBase<'surface.error', { message: string; details?: unknown }>>>();
  let revision = 0;
  let lastSnapshot: SurfaceEnvelopeBase<'surface.snapshot', TreeDataWindowSnapshot> | null = null;
  let closed = false;
  ...
  async function ensureSnapshotListener(): Promise<void> {
    ...
    unlistenSnapshot = await listen<SurfaceEnvelopeBase<'surface.snapshot', TreeDataWindowSnapshot>>(
      getTreeDataBridgeSnapshotEventName(instanceId),
      (event) => {
        if (closed) {
          return;
        }
        ...
        log('snapshot received', {
          ...
          instanceId,
          revision,
          selectedNodeId: event.payload.payload.viewState.selectedNodeId,
          expandedNodeCount: event.payload.payload.viewState.expandedNodeIds.length,
        });
  ...
  return {
    ...
    getSnapshot(): SurfaceEnvelopeBase<'surface.snapshot', TreeDataWindowSnapshot> | null {
      return lastSnapshot;
    },
    isClosed(): boolean {
      return closed;
    ...
    sendCommand(
      payload: TreeDataWindowCommand,
      options: SurfaceTransportCommandOptions = {},
    ): SurfaceEnvelopeBase<'surface.command', TreeDataWindowCommand> | null {
      if (closed) {
        return null;
      }

      const envelope: SurfaceTransportBridgeCommandEnvelope<TreeDataWindowCommand> = {
        surfaceId,
        instanceId,
    ...
    publishSnapshot(
      payload: TreeDataWindowSnapshot,
      ...
      const envelope: SurfaceEnvelopeBase<'surface.snapshot', TreeDataWindowSnapshot> = {
        surfaceId,
        instanceId,
      ...
      log('publish snapshot locally', {
        ...
        instanceId,
        revision,
        selectedNodeId: payload.viewState.selectedNodeId,
        expandedNodeCount: payload.viewState.expandedNodeIds.length,
      });
    ...
    onCommand(
      listener: SurfaceTransportListener<SurfaceEnvelopeBase<'surface.command', TreeDataWindowCommand>>,
    ): SurfaceTransportUnlisten {
    ...
    onSnapshot(
      listener: SurfaceTransportListener<SurfaceEnvelopeBase<'surface.snapshot', TreeDataWindowSnapshot>>,
    ): SurfaceTransportUnlisten {
