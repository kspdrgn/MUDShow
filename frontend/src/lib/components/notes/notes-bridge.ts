...
import type { NotesWindowCommand, NotesWindowSnapshot } from './notes-transport';
...
export type NotesBridgeSession = SurfaceTransportSession<NotesWindowCommand, NotesWindowSnapshot>;

export function getNotesBridgeSnapshotEventName(instanceId: SurfaceInstanceId): string {
  return `${BRIDGE_SNAPSHOT_EVENT_PREFIX}${instanceId}`;
}

export function createNotesBridgeSession(
  surfaceId: SurfaceId,
  instanceId: SurfaceInstanceId,
): NotesBridgeSession {
  const snapshotListeners = new Set<SurfaceTransportListener<SurfaceEnvelopeBase<'surface.snapshot', NotesWindowSnapshot>>>();
  const lifecycleListeners = new Set<SurfaceTransportListener<SurfaceEnvelopeBase<'surface.lifecycle', SurfaceLifecycleEvent>>>();
  const commandListeners = new Set<SurfaceTransportListener<SurfaceEnvelopeBase<'surface.command', NotesWindowCommand>>>();
  const errorListeners = new Set<SurfaceTransportListener<SurfaceEnvelopeBase<'surface.error', { message: string; details?: unknown }>>>();
  let revision = 0;
  let lastSnapshot: SurfaceEnvelopeBase<'surface.snapshot', NotesWindowSnapshot> | null = null;
  let closed = false;
  ...
  async function ensureSnapshotListener(): Promise<void> {
    ...
    unlistenSnapshot = await listen<SurfaceEnvelopeBase<'surface.snapshot', NotesWindowSnapshot>>(
      getNotesBridgeSnapshotEventName(instanceId),
      (event) => {
        if (closed) {
          return;
        }
        ...
        log('snapshot received', {
          ...
          instanceId,
          revision,
          noteLength: event.payload.payload.model.notes.length,
        });
  ...
  return {
    ...
    getSnapshot(): SurfaceEnvelopeBase<'surface.snapshot', NotesWindowSnapshot> | null {
      return lastSnapshot;
    },
    isClosed(): boolean {
      return closed;
    ...
    sendCommand(
      payload: NotesWindowCommand,
      options: SurfaceTransportCommandOptions = {},
    ): SurfaceEnvelopeBase<'surface.command', NotesWindowCommand> | null {
      if (closed) {
        return null;
      }

      const envelope: SurfaceTransportBridgeCommandEnvelope<NotesWindowCommand> = {
        surfaceId,
        instanceId,
    ...
    publishSnapshot(
      ...
      const envelope: SurfaceEnvelopeBase<'surface.snapshot', NotesWindowSnapshot> = {
        surfaceId,
        instanceId,
      ...
      log('publish snapshot locally', {
        ...
        instanceId,
        revision,
        noteLength: payload.model.notes.length,
      });
    ...
    onCommand(
      listener: SurfaceTransportListener<SurfaceEnvelopeBase<'surface.command', NotesWindowCommand>>,
    ): SurfaceTransportUnlisten {
    ...
    onSnapshot(
      listener: SurfaceTransportListener<SurfaceEnvelopeBase<'surface.snapshot', NotesWindowSnapshot>>,
    ): SurfaceTransportUnlisten {
