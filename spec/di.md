...
- `notes` for working note text, loading, debounced saves, and close-time flushes
- `transcript` for character history loading, rolling append/trim coordination, and persistence requests
...
The notes service owns only live working text and save coordination; saved
notes remain character-owned app storage. The transcript service owns only
history coordination; the canonical live transcript remains frontend-owned.
Plugin-specific disposable caches, such as the FuzzBall property tree, belong
to the plugin's session contribution and are exposed through its typed service
bag rather than through a global cache.
...
- Use `notes` methods for session working text and save coordination. Flush it before a world tab is closed when existing close behavior requires it.
- Use `transcript` methods for configured character-history loading and rolling persistence. Do not use it as a replacement for the canonical live transcript.
- Use `style` methods to resolve the effective style for the active world session.
- Obtain plugin-owned runtime caches from the plugin service bag; cache contents may be discarded and queried again.
